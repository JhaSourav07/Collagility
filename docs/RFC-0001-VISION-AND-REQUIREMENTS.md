...
* **FR-1.1 (Session Creation) (done):** The host CLI must be able to spawn a new collaborative session bound to a local AI agent CLI process (Gemini CLI, Claude, Codex, Aider, Goose).
* **FR-1.2 (Session Join) (done):** Participants must be able to join an active session using a unique, cryptographically random Session ID or connection link (`collagility join <session-id>`).
* **FR-1.3 (Role & Permission Control) (done):** The host must be able to dynamically grant/revoke permissions per participant:
  * *Host / Owner (done):* Full control over AI execution, session lifecycle, driver privileges, and participant eviction.
  * *Co-Driver (Read/Write) (future):* Interactive peer prompt submission modal with real-time driver handoff.
  * *Observer (Read-Only) (done):* Permission to view context, AI responses, presence, and chat, but blocked from issuing agent prompts.
* **FR-1.4 (Host Local Isolation) (done):** All LLM API calls, key management, file system access, and command executions generated by the AI agent MUST execute solely on the host OS.

### 11.2 Real-Time Event Synchronization
* **FR-2.1 (Stream Multiplexing) (done):** The system must stream AI agent stdout/stderr, token generation chunks, tool execution statuses, and context updates to all connected participants in real time.
* **FR-2.2 (Context Payload Transparency) (done):** Participants must be able to view system prompts, file snippets, and conversation history injected into the AI agent.
* **FR-2.3 (Diff & File Preview Sync) (done):** Proposed code modifications, file diffs, and staging changes produced by the AI agent must be rendered as synchronized syntax-highlighted diffs across all client terminals.

### 11.3 Presence & Communication
* **FR-3.1 (Participant Presence) (done):** Real-time status bar displaying connected participants, their roles, latency ping, and active focus state.
* **FR-3.2 (Out-of-Band Chat) (done):** A dedicated terminal side-chat channel that allows human participants to communicate without altering or dirtying the AI session prompt buffer.
* **FR-3.3 (Typing & Action Indicators) (done):** Visual cues when a participant is drafting a prompt or when the host machine is executing an AI-generated command.

### 11.4 Agent Driver Integration Layer
* **FR-4.1 (Gemini & Multi-Provider CLI Adapters) (done):** A modular driver wrapper capable of interfacing with Gemini CLI, Claude, Codex, Aider, and Goose sub-processes via standard I/O pipes and structured JSON/event streams.
* **FR-4.2 (Abstract Driver Interface) (done):** Clean separation of the agent driver interface (`IAgentDriver`) allowing rapid onboarding of AI agent CLI binaries.
...
The initial release (v0.1.0) focuses on establishing a rock-solid, secure, low-latency multiplayer foundation targeting local CLI agents.

### Included in MVP:
* **Core CLI App (`collagility`) (done):** Single binary supporting both `start` and `join` execution modes.
* **Multi-Provider CLI Adapters (done):** Native I/O interception, streaming parser, and process hooks for Gemini CLI, Claude, Codex, Aider, and Goose.
* **Open-Source Relay Server (`collagility server`) (done):** Fastify WebSocket-based pub/sub server with docker support.
* **Terminal Stream Renderer (done):**
  * Main AI prompt/response streaming view **(done)**.
  * Side-channel participant chat prompt **(done)**.
  * Participant presence and status indicator bar **(done)**.
* **Role Management (done):** Host-controlled permission toggles (Owner vs. Participant).
* **Session Security (done):** Random session IDs and TLS WebSocket transport encryption.
...
### Phase 2: Multi-Agent Driver Expansion & IDE Integration
* **IDE Extension Plugins (future):** VS Code & Antigravity IDE plugins for GUI-based pairing.
* **Interactive Co-Driver Approvals (future):** Peer prompt submission and approval modal for agent tool calls.

### Phase 3: Zero-Trust Security & Web Access
* **End-to-End Encryption (E2EE) (future):** Host-generated ephemeral keypair exchange.
* **Web Browser Observer Client (future):** WebAssembly/Xterm.js viewer.

### Phase 4: Session Analytics & Replay
* **Session Recording & Playback (future):** Event log recording for asynchronous code reviews.
* **Multi-Agent Consensus Swarm (future):** Routing prompts across multiple agent models simultaneously.

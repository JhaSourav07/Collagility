...
export class GooseAIAdapter extends AIAdapter {
  public readonly id = 'adapter-goose-cli';
  ...
  public async sendPrompt(
    ...
    const responseText = `[Goose Adapter Stub Response]: Processed prompt "${prompt}"`;
    this._status = 'ready';

    const evt = createAICompletedEvent(this.name, responseText, { provider: 'block-goose' });
  ...
  public async health(): Promise<AdapterHealth> {
    return {
      ...
      message: `Goose adapter status: ${this._status}`,
    };
  }
